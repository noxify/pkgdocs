...
/**
 * Returns the root collections from the source collection.
 ...
 */
export async function rootCollections(sourceCollection: SourceCollection) {
  ...
  return await Promise.all(
    roots.filter(isDirectory).map(async (collection) => {
      const file = await getFileContent(sourceCollection, collection)
      const frontmatter = await getMetadata(file)

      return {
        title: getTitle(collection, frontmatter, true),
        entrypoint:
...
export async function transformedEntries(
  ...
  return await Promise.all(allEntries.map((entry) => getEntry(sourceCollection, entry)))
}
 ...
 */
export async function getFileContent(sourceCollection: SourceCollection, source: EntryType) {
 ...
 */
export async function getMetadata(file: Awaited<ReturnType<typeof getFileContent>>) {
 ...
 * @returns Promise resolving to the file object or null if not found
 */
export async function getFileForEntry(
  sourceCollection: SourceCollection,
...
export async function getRawEntry(
  sourceCollection: SourceCollection,
  transformedEntry: TransformedEntry,
): Promise<Awaited<ReturnType<SourceCollection["getEntry"]>>> {
