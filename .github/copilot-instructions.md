...
- **Prefer global imports**: use global imports instead of local imports ( e.g. `const x = await import("../some-module")` )
...
- **Import order**: Follow the specified import grouping:
  1. Types first
  2. React/Next.js/Waku/Vite/Expo/Remote (if applicable)
...
- **Generic type parameters**: Always prefix with `T` (e.g., `TResult`)
...
- Do not try to create a file via terminal commands (e.g., `touch`, `echo`), instead generate the entire file directly
